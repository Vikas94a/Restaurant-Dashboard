...
import { isItemAvailable, AvailabilityState } from "@/features/menu/types/availability";
...
export default function RestaurantMenu({ restaurantId }: RestaurantMenuProps) {
  ...
  const parseMenuItem = useCallback((item: any, categoryId: string, itemIndex: number): NestedMenuItem | null => {
    ...
    return {
      ...
      isPopular: Boolean(item.isPopular), // Include isPopular flag
      availability: item.availability || (item.isAvailable ? {
        status: 'available' as const,
        until: null,
      } : {
        status: 'unavailable_indefinite' as const,
        until: null,
      }),
  ...
  const fetchMenuData = useCallback(async () => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        const allItems = (data.items || []).map((item: any, index: number) => 
          parseMenuItem(item, doc.id, index)
        ).filter(Boolean) as NestedMenuItem[];
        
        // Filter out unavailable items for customer menu
        const availableItems = allItems.filter((item) => {
          const availability: AvailabilityState = item.availability || {
            status: item.isAvailable ? 'available' as const : 'unavailable_indefinite' as const,
            until: null,
          };
          return isItemAvailable(availability);
        });
