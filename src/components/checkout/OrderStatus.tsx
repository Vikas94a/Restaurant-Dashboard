...
const OrderStatus: React.FC<OrderStatusProps> = ({
  ...
  return (
    <div className="fixed inset-0 backdrop-filter backdrop-blur-sm bg-black bg-opacity-30 flex items-center justify-center z-50">
          ...
          {placedOrder.status === 'accepted' && (
            <>
              ...
              {placedOrder.estimatedPickupTime && (
                <p className="text-gray-600 mt-2">Estimated pickup time: <span className="font-medium text-green-700">{placedOrder.estimatedPickupTime}</span></p>
              )}
              {!placedOrder.estimatedPickupTime && (
